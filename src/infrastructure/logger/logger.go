...
var (
	...
	mobileloketLog *rotatelogs.RotateLogs
...
func init() {
	...
	// default mobileloket log dir setting
	...
	// Set mobileloket logging.
	mobileloketLog, _ = rotatelogs.New(
		logdir+"/mobileloketlog.%Y%m%d%H%M",
		rotatelogs.WithLinkName(logdir+"/mobileloket_log"),
		rotatelogs.WithMaxAge(time.Duration(logMaxAge)*24*time.Hour),
		rotatelogs.WithRotationTime(time.Hour),
	)
	...
	Logger = zerolog.New(mobileloketLog).With().Timestamp().Logger()
	//Logger = log.New(mobileloketLog, "mobileloket:", log.Lshortfile)
}
